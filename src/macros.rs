...
macro_rules! get_set_command {
    ...
    (
        ...
        get_set_command!(get $get $text $set_name);
        get_set_command!(set $set $text $set_name);
        get_set_command!(params $set_name $set_error $set_fields);
    };
    // Param set
    (
        params $set_name:ident ($set_error_name:ident) (
            $($field_name:ident: $kind:tt $type_name:ident $body:tt),* $(,)?
        )
    ) => {
        $(get_set_command!(param $kind $type_name $body);)*
    ...
    (param enum $name:ident { $($variant:ident => $val:literal),* $(,)? }) => {
    ...
    // Commands
    (get $name:ident $text:literal $response:ident) => {
        pub struct $name;
    ...
    (set $name:ident $text:literal $params:ident) => {
